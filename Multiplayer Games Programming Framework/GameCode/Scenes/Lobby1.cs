...
internal class Lobby1 : Scene
{
    ...
    // Constructor
    public Lobby1(SceneManager manager) : base(manager)
    {
        manager.m_Game.IsMouseVisible = true;

        if (NetworkManager.m_Instance != null)
            // Send a packet to the server to say that the client is in lobby 1
            NetworkManager.m_Instance.TcpSendPacket(new inLobby(NetworkManager.m_Instance.m_clientID, 1), true);
    }
    ...
    public override void LoadContent()
    {
        ...
        // Ready button to ready up for the game
        ...
        m_readyButton.Click += (s, a) =>
        {
            ...
            if (NetworkManager.m_Instance != null)
                NetworkManager.m_Instance.TcpSendPacket(new startLobby(m_ready, 1), true);
        };

        // Back button to go back to the lobby select screen
        ...
        m_backButton.Click += (s, a) =>
        {
            ...
            if (NetworkManager.m_Instance != null)
            {
                // Send a packet to the server to say that the client is no longer in lobby 1
        ...
        // Change colour button to change the colour of the player
        ...
        m_changeColour.Click += (s, a) =>
        {
            ...
            // Switch statement to change the colour of the player
    ...
    {
        // If the game is not active then disable the buttons
        ...
        // If the lobby has not been started by the server then return
        if (NetworkManager.m_Instance != null && !NetworkManager.m_Instance.m_startLobby1) return;
        m_Manager.LoadScene(new GameScene(m_Manager));
    }

    public override void Draw(float deltaTime)
    {
        // Draw the lobby 1 text
        m_Manager.m_Game.m_SpriteBatch.DrawString(m_Manager.m_Game.m_Spritefont, "Lobby 1", new Vector2(-490, -270),
            Color.White);

        if (NetworkManager.m_Instance != null)

            // Switch statement to display the players that have joined the lobby
