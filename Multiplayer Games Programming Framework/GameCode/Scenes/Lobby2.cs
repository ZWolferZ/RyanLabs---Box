...
internal class Lobby2 : Scene
{
    ...
    public Lobby2(SceneManager manager) : base(manager)
    {
        manager.m_Game.IsMouseVisible = true;
        if (NetworkManager.m_Instance != null)
            NetworkManager.m_Instance.TcpSendPacket(new inLobby(NetworkManager.m_Instance.m_clientID, 2), true);
    }
    ...
    public override void LoadContent()
    {
        ...
        m_readyButton.Click += (s, a) =>
        {
            ...
            if (NetworkManager.m_Instance != null)
                NetworkManager.m_Instance.TcpSendPacket(new startLobby(m_ready, 2), true);
        };
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (NetworkManager.m_Instance != null && !NetworkManager.m_Instance.m_startLobby2) return;
        m_Manager.LoadScene(new GameScene(m_Manager));
    }

    public override void Draw(float deltaTime)
    {
        m_Manager.m_Game.m_SpriteBatch.DrawString(m_Manager.m_Game.m_Spritefont, "Lobby 2", new Vector2(-490, -270),
            Color.White);
